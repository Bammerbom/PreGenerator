...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
	...
	public List<Chunk> getChunks() {
		...
		int current = 0;
		int max = (this.getSizeX() / 16) * (this.getSizeZ() / 16);
		max = max + (((this.getSizeX() / 16) + (this.getSizeZ() / 16)) * 2) + 1;
		Main.max = max;
		for (int x = x1; x <= x2; x += 16) {
			for (int z = z1; z <= z2; z += 16) {
				res.add(w.getChunkAt(x >> 4, z >> 4));
				Main.log("Calculating chunks... (" + current + "/" + max + ")");
				current++;
			}
		}
		Main.max = current;
	...
	public void generateChunks(){
		...
		for (int x = x1; x <= x2; x += 16) {
			for (int z = z1; z <= z2; z += 16) {
				...
				if(current % 1000 == 0){
				Main.log("Generating chunks... (" + current + "/" + max + ") [" + ( (Math.round(((current * 100.0F) / (max / 1.0F) * 10.0F)) / 10.0F) + "%]") + " (RAM: " + ( (Math.round((((Runtime.getRuntime().maxMemory() - Runtime.getRuntime().freeMemory()) * 100.0F) / (Runtime.getRuntime().maxMemory() / 1.0F) * 10.0F)) / 10.0F) + "%)"));
				}
				current++;
				if(( (Math.round((((Runtime.getRuntime().maxMemory() - Runtime.getRuntime().freeMemory()) * 100.0F) / (Runtime.getRuntime().maxMemory() / 1.0F) * 10.0F)) / 10.0F) > 80.0F)){
					while(( (Math.round((((Runtime.getRuntime().maxMemory() - Runtime.getRuntime().freeMemory()) * 100.0F) / (Runtime.getRuntime().maxMemory() / 1.0F) * 10.0F)) / 10.0F) > 60.0F)){
						try {
							Main.log("Memory usage too high, waiting... [" + ( (Math.round(((current * 100.0F) / (max / 1.0F) * 10.0F)) / 10.0F) + "%]") + " (RAM: " + ( (Math.round((((Runtime.getRuntime().maxMemory() - Runtime.getRuntime().freeMemory()) * 100.0F) / (Runtime.getRuntime().maxMemory() / 1.0F) * 10.0F)) / 10.0F) + "%)"));
							System.gc();
							Thread.sleep(5000);
						} catch (InterruptedException e) {
							e.printStackTrace();
